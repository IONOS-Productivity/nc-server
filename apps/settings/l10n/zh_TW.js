OC.L10N.register(
    ...
    "Logged in account must be a sub admin" : "登入的帳號必須為子管理員",
    "Apps" : "應用程式",
    "Personal" : "個人",
    "Administration" : "管理",
    "Users" : "使用者",
    ...
    "Logging level" : "紀錄層級",
    "The %1$s configuration option must be a valid integer value." : "設定選項 %1$s 必須是有效的整數值。",
    "The logging level is set to debug level. Use debug level only when you have a problem to diagnose, and then reset your log level to a less-verbose level as it outputs a lot of information, and can affect your server performance." : "紀錄層級已設定為除錯層級。請僅在您需要診斷問題時使用除錯層級，然後將紀錄層級重設為冗餘較少的層級，因為除錯層集會產生大量資訊，並可能影響伺服器效能。",
    "Logging level configured correctly." : "紀錄層級設定正確。",
    ...
    "Allow users to set custom share link tokens" : "允許使用者設定自訂分享連結權杖",
    ...
    "Also allow autocompletion on full match of the display name" : "同時允許在顯示名稱完全相符時啟用自動完成功能",
    ...
    "Search groups…" : "搜尋群組……",
    "List of groups. This list is not fully populated for performance reasons. The groups will be loaded as you navigate or search through the list." : "群組清單。出於效能考量，此清單並未完全呈現。群組將會在您於清單中瀏覽或搜尋時載入。",
    "Loading groups…" : "正在載入群組……",
    ...
    "To encrypt all existing files run this OCC command:" : "要加密所有現有檔案，請執行 OCC 命令：",
    "Enable server-side encryption" : "啟用伺服器端加密",
    "Disabling server side encryption is only possible using OCC, please refer to the documentation." : "停用伺服器端加密僅能使用 OCC，請參考相關說明文件。",
    ...
    "By default a master key for the whole instance will be generated. Please check if that level of access is compliant with your needs." : "預設情況下，將會產生整個站台的主控金鑰。請檢查存取權限是否符合您的需求。",
    ...
    "Delete" : "刪除",
    "Reshare" : "轉發分享",
    "Default config values" : "預設設定值",
    "Applications config" : "應用程式設定",
    "User config" : "使用者設定",
    "Bundled applications" : "隨附應用程式",
    "Enabled applications" : "已啟用的應用程式",
    "Disabled applications" : "已停用的應用程式",
    "Large organization" : "超大型組織",
    "Big organization" : "大型組織",
    "Small organization" : "小型組織",
    "Hosting company" : "託管公司",
    "University" : "大學",
    "School" : "學校",
    "Club or association" : "俱樂部或協會",
    "Family" : "家族",
    "Personal use" : "個人使用",
    "Setup errors" : "設定錯誤",
    "Setup warnings" : "設定警告",
    ...
    "In case of lost device or exiting the organization, this can remotely wipe the {productName} data from all devices associated with {userid}. Only works if the devices are connected to the internet." : "如果遺失裝置或退出組織，可以從與 {userid} 相關的所有裝置遠端抹除 {productName} 資料。僅當裝置連線到網際網路時才有效。",
    ...
    "For performance reasons, when you enable encryption on a {productName} server only new and changed files are encrypted." : "出於效能考量，當您在 {productName} 上啟用加密時，僅會加密新檔案與已變更的檔案。",
    "Location" : "位置",
