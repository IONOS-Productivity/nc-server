OC.L10N.register(
    ...
    "List of invalid files…" : "잘못된 파일들의 목록…",
    "Rescan…" : "다시 스캔…",
    "Cron errors" : "Cron 오류",
    ...
    "Email test" : "이메일 시험",
    "Mail delivery is disabled by instance config \"%s\"." : "인스턴스의 \"%s\" 설정에 의해 메일 전달이 비활성화 되었습니다.",
    ...
    "Your webserver is not set up to serve `.js.map` files. Without these files, JavaScript Source Maps won't function properly, making it more challenging to troubleshoot and debug any issues that may arise." : "당신의 웹서버가 `.js.map` 파일을 제공하도록 설정되지 않았습니다. 이 파일들 없이는 JavaScript Source Maps가 올바르게 작동하지 않으며, 발생할 문제들을 해결하고 디버깅하기 더 어려워집니다.",
    ...
    "Logging level" : "로그 단계",
    "The %1$s configuration option must be a valid integer value." : "%1$s설정 옵션은 유효한 정수 값이어야만 합니다.",
    "The logging level is set to debug level. Use debug level only when you have a problem to diagnose, and then reset your log level to a less-verbose level as it outputs a lot of information, and can affect your server performance." : "로그 단계가 디버그 단계로 설정되어 있습니다. 디버그 단계는 문제를 진단할 때만 사용하고, 문제가 해결되면 로그 단계를 더 간략한 단계로 되돌리십시오. 디버그 단계는 많은 정보를 출력하여 서버 성능에 영향을 줄 수 있습니다.",
    "Logging level configured correctly." : "로그 단계가 올바르게 설정되었습니다.",
    ...
    "Memcache" : "Memcache",
    "Memcached is configured as distributed cache, but the wrong PHP module (\"memcache\") is installed. Please install the PHP module \"memcached\"." : "Memcached가 분산 캐시로 구성되어 있지만 잘못된 PHP 모듈 (\"memcache\")가 설치되어 있습니다. PHP 모듈 \"memcached\"를 설치하세요.",
    "Memcached is configured as distributed cache, but the PHP module \"memcached\" is not installed. Please install the PHP module \"memcached\"." : "Memcached가 분산 캐시로 구성되어 있지만 PHP 모듈 \"memcached\"가 설치되어 있지 않습니다. PHP 모듈 \"memcached\"를 설치하세요.",
    ...
    "Failed to write and read a value from local cache." : "로컬 캐시에 값을 쓰거나 읽을 수 없습니다.",
    "Failed to write and read a value from distributed cache." : "분산 캐시에 값을 쓰거나 읽을 수 없습니다.",
    "Configured" : "설정됨",
    "Mimetype migrations available" : "Mimetype 마이그레이션 가능",
    "One or more mimetype migrations are available. Occasionally new mimetypes are added to better handle certain file types. Migrating the mimetypes take a long time on larger instances so this is not done automatically during upgrades. Use the command `occ maintenance:repair --include-expensive` to perform the migrations." : "하나 이상의 MIME 유형 마이그레이션이 가능합니다. 특정 파일 형식을 더 잘 처리하기 위해 새로운 MIME 유형이 추가되는 경우가 있습니다. 대규모 인스턴스에서는 MIME 유형 마이그레이션에 시간이 오래 걸리므로 업그레이드 시 자동으로 수행되지 않습니다. 마이그레이션을 수행하려면 `occ maintenance:repair --include-expensive` 명령을 사용하십시오.",
    "MySQL row format" : "MySQL 행 형식",
    "You are not using MySQL" : "MySQL을 사용하고 있지 않습니다",
    "None of your tables use ROW_FORMAT=Compressed" : "ROW_FORMAT=Compressed인 테이블이 없습니다.",
    "Incorrect row format found in your database. ROW_FORMAT=Dynamic offers the best database performances for Nextcloud. Please update row format on the following list: %s." : "데이터베이스에서 잘못된 행 형식이 발견되었습니다. ROW_FORMAT=Dynamic은 Nextcloud에서 최상의 데이터베이스 성능을 제공합니다. 다음 목록에서 행 형식을 업데이트하십시오: %s",
    ...
    "OCS provider resolving" : "OCS 제공자 해석",
    "Could not check if your web server properly resolves the OCM and OCS provider URLs." : "웹 서버가 OCM 및 OCS 공급자 URL을 제대로 해석하는지 확인할 수 없습니다.",
    "Your web server is not properly set up to resolve %1$s.\nThis is most likely related to a web server configuration that was not updated to deliver this folder directly.\nPlease compare your configuration against the shipped rewrite rules in \".htaccess\" for Apache or the provided one in the documentation for Nginx.\nOn Nginx those are typically the lines starting with \"location ~\" that need an update." : "웹 서버가 %1$s를 제대로 해석하도록 설정되어 있지 않습니다. \n이 문제는 웹 서버 구성이 해당 폴더를 직접 제공하도록 업데이트되지 않았기 때문일 가능성이 높습니다. \nApache의 경우 \".htaccess\" 파일에 있는 기본 rewrite 규칙과, Nginx의 경우 설명서에 제공된 rewrite 규칙과 현재 구성을 비교해 보세요.\nNginx의 경우 일반적으로 \"location ~\"로 시작하는 줄을 업데이트해야 합니다.",
    ...
    "PHP APCu configuration" : "PHP APCu 설정",
    "Your APCu cache has been running full, consider increasing the apc.shm_size php setting." : "APCu 캐시가 가득 찼으므로, php 설정에서 apc.shm_size를 늘리는 것을 고려해 보세요.",
    "Your APCu cache is almost full at %s%%, consider increasing the apc.shm_size php setting." : "APCu 캐시가 %s%%정도로 거의 가득 찼으므로, php 설정에서 apc.shm_size를 늘리는 것을 고려해 보세요.",
    ...
    "PHP file size upload limit" : "PHP 파일 크기 업로드 제한",
    "The PHP upload_max_filesize is too low. A size of at least %1$s is recommended. Current value: %2$s." : "PHP의 upload_max_filesize가 너무 작습니다. 최소한 %1$s 이상의 크기를 권장합니다. 현재 값: %2$s.",
    "The PHP post_max_size is too low. A size of at least %1$s is recommended. Current value: %2$s." : "PHP의 post_max_filesize가 너무 작습니다. 최소한 %1$s 이상의 크기를 권장합니다. 현재 값: %2$s.",
