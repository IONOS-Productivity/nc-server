OC.L10N.register(
    ...
    "Logged in account must be a sub admin" : "登录的账号必须是子管理员",
    "Apps" : "应用",
    "Personal" : "个人",
    "Administration" : "管理",
    "Users" : "用户",
    ...
    "Accessing site insecurely via HTTP. You are strongly advised to set up your server to require HTTPS instead. Without it some important web functionality like \"copy to clipboard\" or \"service workers\" will not work!" : "通过 HTTP 访问网站存在安全隐患。强烈建议您将服务器设置为使用 HTTPS。否则，部分重要网络功能（例如“复制到剪贴板”或“服务工作进程”）将无法正常工作‌！",
    ...
    "Logging level configured correctly." : "日志级别配置正确。",
    ...
    "Could not check if your web server properly resolves the OCM and OCS provider URLs." : "无法检查您的 Web 服务器是否正确解析 OCM 和 OCS 提供者 URL。",
    ...
    "The OPcache buffer is nearly full. To assure that all scripts can be hold in cache, it is recommended to apply \"opcache.memory_consumption\" to your PHP configuration with a value higher than \"%s\"." : "OPcache 缓冲区几乎已满。 为了确保所有脚本都可以保存在缓存中，建议将“opcache.memory_consumption”应用到您的 PHP 配置，其值高于 \"%s\".",
    ...
    "Task Processing pickup speed" : "任务处理领取速度",
    ...
    "Allow users to set custom share link tokens" : "允许用户设置自定义共享链接令牌",
    "Shares with custom tokens will continue to be accessible after this setting has been disabled" : "禁用此设置后，可继续访问具有自定义令牌的共享",
    "Shares with guessable tokens may be accessed easily" : "可以轻松访问带有可猜测令牌的共享",
    "Limit sharing based on groups" : "基于组限制共享",
    "Allow sharing for everyone (default)" : "允许所有人共享（默认）",
    "Exclude some groups" : "排除部分组",
    "Allow some groups" : "允许部分组",
    ...
    "Allow account name autocompletion in share dialog and allow access to the system address book" : "允许在共享对话框中自动补全账号名称并允许访问系统通讯录",
    "Sharing autocompletion restrictions" : "共享自动补全限制",
    "If autocompletion restrictions for both \"same group\" and \"phonebook integration\" are enabled, a match in either is enough to show the user." : "如果同时启用了“同组”和“通讯录集成”的自动补全限制，则其中任何一个匹配即会向用户显示。",
    "Restrict account name autocompletion and system address book access to users within the same groups" : "将账号名称自动补全和系统通讯录访问限制为同组内的用户",
    ...
    "Failed to load groups" : "无法加载群组",
    "Failed to create group" : "无法创建群组",
    "Groups" : "分组",
    ...
    "Search groups…" : "搜索群组…",
    "List of groups. This list is not fully populated for performance reasons. The groups will be loaded as you navigate or search through the list." : "群组列表。出于性能原因，此列表未完全填充。当您在列表中导航或搜索时，将加载这些群组。",
    ...
    "This app has no minimum {productName} version assigned. This will be an error in the future." : "此应用未分配最低 {productName} 版本。这会导致以后出现错误。",
    "This app has no maximum {productName} version assigned. This will be an error in the future." : "此应用未分配最高 {productName} 版本。这会导致以后出现错误。",
    ...
    "To encrypt all existing files run this OCC command:" : "要加密全部现有文件，请运行 OCC 命令：",
    "Enable server-side encryption" : "启用服务端加密",
    "Disabling server side encryption is only possible using OCC, please refer to the documentation." : "只有使用 OCC 才能禁用服务端加密，请参阅文档。",
    ...
    "Delete" : "删除",
    "Reshare" : "再次共享",
    "Default config values" : "默认配置值",
    "Applications config" : "应用程序配置",
    "User config" : "用户配置",
    "Bundled applications" : "已捆绑的应用程序",
    "Enabled applications" : "已启用的应用程序",
    "Disabled applications" : "已禁用的应用程序",
    "Large organization" : "大型组织",
    "Big organization" : "大型组织",
    "Small organization" : "小型组织",
    ...
    "In case of lost device or exiting the organization, this can remotely wipe the {productName} data from all devices associated with {userid}. Only works if the devices are connected to the internet." : "如果设备丢失或退出组织，这可以远程擦除与 {userid} 关联的所有设备中的 {productName} 数据。仅当设备连接到互联网时才有效。",
    ...
    "Failed to send email" : "无法发送邮件",
    "Failed to save email settings" : "无法保存电子邮件设置",
    ...
    "The server configuration is read-only so the mail settings cannot be changed using the web interface." : "服务器配置是只读的，因此无法使用 Web 界面更改邮件设置。",
    "Mail delivery is disabled by instance config \"{config}\"." : "实例配置“{config}”已禁用邮件传递。",
    "Send mode" : "发送模式",
    "Encryption" : "加密",
    ...
    "Sending test email…" : "正在发送测试邮件…",
    "Send test email" : "发送测试邮件",
    "Saving…" : "正在保存…",
    "Save settings" : "保存设置",
    "Please double check the {linkStartInstallationGuides}installation guides{linkEnd}, and check for any errors or warnings in the {linkStartLog}log{linkEnd}." : "请仔细检查{linkStartInstallationGuides}安装指南{linkEnd}，并检查{linkStartLog}日志{linkEnd}中是否有任何错误或警告。",
    "Check the security of your {productName} over {linkStart}our security scan{linkEnd}." : "通过{linkStart}安全扫描{linkEnd}检查 {productName} 的安全性。",
    "Security & setup warnings" : "安全与设置警告",
    "It is important for the security and performance of your instance that everything is configured correctly. To help you with that we are doing some automatic checks. Please see the linked documentation for more information." : "正确配置所有内容对于实例的安全性和性能非常重要。为了帮助您，我们正在进行一些自动检查。如需了解详情，请参阅链接的文档。",
    "Checking your server …" : "正在检查您的服务器 …",
    "Failed to run setup checks" : "无法运行设置检查",
    "Try again" : "重试",
    ...
    "Failed to save selected preset." : "无法保存所选预设。",
    "Select a configuration preset for easy setup." : "选择配置预设以便于设置。",
    "Apply" : "应用",
    "All accounts" : "所有账号",
    "Admins" : "管理员",
    ...
    "This app is not marked as compatible with your {productName} version." : "此应用未标记为与您的 {productName} 版本兼容。",
    "If you continue you will still be able to install the app. Note that the app might not work as expected." : "如果继续，您仍然可以安装该应用。请注意，该应用可能无法按预期工作。",
    "Unknown" : "未知",
    "Never" : "从不",
    ...
    "Follow us on Bluesky" : "在 Bluesky 上关注我们",
