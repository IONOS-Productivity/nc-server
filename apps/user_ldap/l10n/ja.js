OC.L10N.register(
    ...
    "Not recommended, use it for testing only! If connection only works with this option, import the LDAP server's SSL certificate in your {instanceName} server." : "推奨されません、テストにおいてのみ使用してください！このオプションでのみ接続が動作する場合は、LDAP サーバーのSSL証明書を {instanceName} サーバーにインポートしてください。",
    ...
    "Groups meeting these criteria are available in {instanceName}:" : "これらの基準を満たすグループが {instanceName} で利用可能:",
    "Only these object classes:" : "このオブジェクトクラスからのみ:",
    "Only from these groups:" : "これらのグループからのみ:",
    "Edit LDAP Query" : "LDAPクエリの編集",
    "The filter specifies which LDAP groups shall have access to the {instanceName} instance." : "フィルターは、どの LDAP グループが {instanceName} にアクセスするかを指定します。",
    "LDAP Filter:" : "LDAP フィルタ：",
    "Verify settings and count the groups" : "設定を検証し、グループを数える",
    "User not found. Please check your login attributes and username. Effective filter (to copy-and-paste for command-line validation): {filter}" : "ユーザーは見つかりませんでした。ログインの属性とユーザー名をチェックしてください。適用されているフィルター(コピーペーストしてコマンドラインでの確認できます): {filter}",
    "User found and settings verified." : "ユーザーが見つかり、設定が検証できました。",
    "Consider narrowing your search, as it encompassed many users, only the first one of whom will be able to log in." : "最初のユーザーだけがログインできるよう、多くのユーザーを網羅しつつ検索を絞るようにしてください。",
    "An unspecified error occurred. Please check log and settings." : "不明なエラーが発生しました。設定とログを確認してください。",
    "The search filter is invalid, probably due to syntax issues like uneven number of opened and closed brackets. Please revise." : "検索フィルターが不正です。恐らく文法の問題で、開き括弧と閉じ括弧がマッチしていません。修正をお願いします。",
    ...
    "The filter specifies which LDAP users shall have access to the {instanceName} instance." : "フィルターは、どのLDAPユーザーが {instanceName} にアクセスするかを指定します。",
    "Verify settings and count users" : "設定を検証し、ユーザーを数える",
    "User count: {usersCount}" : "ユーザー数: {usersCount}",
    ...
    "Advanced" : "詳細設定",
    "Expert" : "エキスパート設定",
    "The PHP LDAP module is not installed, the backend will not work. Please ask your system administrator to install it." : "PHPのLDAPモジュールがインストールされていません。バックエンドは動作しません。システム管理者にインストールを依頼してください。",
    "Select LDAP Config" : "LDAP設定を選択",
